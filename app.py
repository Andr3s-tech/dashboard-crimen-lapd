...
# ============================================================
# 1. CARGA DE DATOS Y MODELOS (se ejecuta una sola vez al arrancar)
# ============================================================
df = pd.read_excel('data/Crime_Data.xlsx')

modelo_edad = joblib.load('modelos/modelo_edad_rf.pkl')          # Regresor: predice Vict Age
modelo_arresto = joblib.load('modelos/modelo_arresto_rf.bin')     # Clasificador: predice Arresto (0/1)
encoders = joblib.load('modelos/encoders_arresto.pkl')            # dict: {'sex': LabelEncoder, 'descent': LabelEncoder}

le_sex = encoders['sex']
le_descent = encoders['descent']

# ============================================================
# 2. TABLAS DE MAPEO (código -> descripción, para que los dropdowns
#    muestren texto legible pero envíen el código al modelo)
# ============================================================
...
# ============================================================
# 3. GRÁFICAS DEL ANALISTA (estáticas, ya calculadas sobre el dataset real)
# ============================================================
...
def predecir(area, crmcd, premisa, arma, sexo, descent, edad, mes, dia, hora):
    ...
    # --- Modelo 1: predicción de edad (regresor) ---
    ...
    # --- Modelo 2: predicción de arresto (clasificador) ---
